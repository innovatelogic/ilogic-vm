...
void IDrawInterfaceUI::DrawController() const
{
	...
	//if (GetAppMain()->IsAEditor() && !GetAppMain()->IsAGame())
	{
		...
		{	
			...
			ViewportInterface *pViewportInterface = m_pCoreSDK->GetExplorerInstance()->GetExplorer2D();
	...
	case MOUSE_LEFT:
		{
			if (m_pNode->m_pValue->IsFocused())
			{
 				ViewportInterface *pInterface = m_pCoreSDK->GetExplorerInstance()->GetExplorer2D();
