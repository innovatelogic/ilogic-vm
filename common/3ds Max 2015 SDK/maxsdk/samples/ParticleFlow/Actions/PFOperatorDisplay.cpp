	...
	*/
	...
	if ((ordCat == geomCat) || (selCat == geomCat))
	{
		...
		for(int runType = 0; runType < 2; runType++)
		{
			...
			typedef std::map<Mesh*, Matrix3Array> MeshPart;
			MeshPart meshParts;
		  	Matrix3 gwTM;
			...
			for(i=0; i<blockCount; ++i) 
			{
				...
				if (pMesh) 
				{
					...
					meshParts[pMesh].push_back(gwTM);
				}
			}

			for (MeshPart::iterator it = meshParts.begin(); it != meshParts.end(); ++it) 
			{ 
				if (NULL != it->first) 
				{
					...
					{
						vertexBufferHandle =  GenerateInstanceData(&(it->second)[0], it->second.size());
					}
					else
					{
						vertexBufferHandle = MaxSDK::Graphics::GenerateInstanceData(shapeSplitter.GetUVData(),&(it->second)[0], it->second.size());
					}
					IMeshDisplay2* pMeshDisplay2 = static_cast<IMeshDisplay2*>(it->first->GetInterface(IMesh_DISPLAY2_INTERFACE_ID)); 
					...
					instanceData.push_back(meshImp);
				} 	
			} 
	...
	for(InstanceDataArray::iterator iter = instanceData.begin(); 
		iter < instanceData.end(); 
		++iter)
	{
		RenderItemHandleArray renderItems;
		iter->pMeshDisplay->GetRenderItems(generateRenderItemsContext,nodeContext,renderItems);
		
		for(size_t i = 0; i < renderItems.GetNumberOfRenderItems(); ++i)
		{
			MaxSDK::Graphics::RenderItemHandle hInstanceItem;
			if (MaxSDK::Graphics::GenerateInstanceRenderItem(hInstanceItem, renderItems.GetRenderItem(i), iter->vertexBufferHandle))
			{
