...
public:
	PolyOpChamferEdge () : mAmount(0.0f), mOpen(false), mSegments(1), mTension(1.0f), mInvert(false), mType(EP_STANDARD_EDGE_CHAMFER), mSmooth(false), mSmoothType(EP_EDGE_CHAMFER_SMOOTH_ALL), mSmoothThreshold(30.0f), mChamferEdgeGrip(NULL) { }
	~PolyOpChamferEdge() { }
	// Implement PolyOperation methods:
...
#define PRIOR_2015_CHAMFER_EDGE_PARAM_COUNT 3
...
IOResult PolyOpChamferEdge::Save (ISave *isave)
{
	...
	// If we're saving pre-Max 2015, we only save the first 3 parameters (amount, open, segments)
	int saveVersion = GetSavingVersion();
	bool preMax2015 = (saveVersion != 0 && saveVersion <= MAX_RELEASE_R16) ? true : false;
	int paramCount = preMax2015 ? PRIOR_2015_CHAMFER_EDGE_PARAM_COUNT : paramList.Count();
...
bool PolyOpChamferEdge::Do (MNMesh & mesh, LocalPolyOpData *pOpData)
{
	...
	if(mType == EP_QUAD_EDGE_CHAMFER) {
		...
		l_ret = mesh.QuadChamfer(mAmount, mSegments, mTension, output, l_flag);
	}
	else {
...
// The number of PolyOpInsetFace parameters for previous versions...
#define PRIOR_2015_POLYOP_INSET_FACE_PARAM_COUNT 2
...
IOResult PolyOpInsetFace::Save (ISave *isave)
{
	...
	// If we're saving pre-Max 2015, we only save the first 2 parameters (type, amount)
	const int saveVersion   = GetSavingVersion();
	const bool preMax2015   = (saveVersion != 0 && saveVersion <= MAX_RELEASE_R16) ? true : false;
	const int paramCount    = preMax2015 ? PRIOR_2015_POLYOP_INSET_FACE_PARAM_COUNT : paramList.Count();
    ...
    //Ignore this parameter for versions of PolyOpInsetFace prior to 2015
    if (false == preMax2015){
	    if (PreserveMapSettings() != NULL) {
		    isave->BeginChunk (kPreserveMapSettings);
		    PreserveMapSettings()->Save (isave);
...
IOResult PolyOpOutlineFace::Save (ISave *isave)
{
	...
	// If we're saving pre-Max 2015, we only save the first parameters (amount)
	const int saveVersion   = GetSavingVersion();
	const bool preMax2015   = (saveVersion != 0 && saveVersion <= MAX_RELEASE_R16) ? true : false;
	const int paramCount    = preMax2015 ? PRIOR_2015_POLYOP_OUTLINE_FACE_PARAM_COUNT : paramList.Count();
    ...
    //Ignore this parameter for versions of PolyOpOutlineFace prior to 2015
    if (false == preMax2015){
	    if (PreserveMapSettings() != NULL) {
		    isave->BeginChunk (kPreserveMapSettings);
		    PreserveMapSettings()->Save (isave);
