...
inline void FastNormalize(Point3& vec)
{
	vec *= FastInvsqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
}
...
void MeshDeformPW::RebuildSelectedWeights(LocalMeshData *localData,int wrapIndex, const Matrix3& meshTM, const Matrix3& baseTM, float falloff, float distance, int faceLimit)
{
...
void MeshDeformPW::BuildCurrentTMFromVertex(TimeValue t, int wrapIndex, Mesh *msh, Point3 *vnorms, const Matrix3& meshTM, const Matrix3& baseTM, const Matrix3& initialBaseTM)
{
	Matrix3 tm(1);
//get node
//get mesh
...
//get our align vec
	...
	int numThreads = threadData.GetNumberOfThreads(MaxSDK::PerformanceTools::ThreadTools::kDeformationThreading, msh->numVerts);
...
void MeshDeformPW::BuildNormals(Mesh *mesh, Tab<Point3> &vnorms, Tab<Point3> &fnorms)
	{
		...
		ZeroMemory(vcount.Addr(0), sizeof(int) * mesh->numVerts);
		Point3* vnormsPtr = vnorms.Addr(0);
		int* vcountPtr = vcount.Addr(0);
		...
		for (int j = 0; j < mesh->numVerts; j++)
		{
			if (vcountPtr[j]!= 0)
			{
				vnormsPtr[j] = vnormsPtr[j]/(float)vcountPtr[j];
