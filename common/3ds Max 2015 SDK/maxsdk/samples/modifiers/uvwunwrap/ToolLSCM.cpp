...
void LSCMClusterData::ComputePins(MeshTopoData *md, UnwrapMod *mod, bool useExistingMapping)
{
	...
	{	
		...
		if ((pVert1 != -1) && (pVert2 != -1))
		{
			...
			bool bPinsShowConditions = (mod->WtIsChecked(ID_LSCM_INTERACTIVE) || mod->WtIsChecked(ID_LSCM_MAP));
...
bool ToolLSCM::Start( bool useExistingMapping, UnwrapMod *mod, Tab<MeshTopoData*> &localData)
{
	...
	{
		if(NULL != localData[ldID])
		{
			BitArray faceSel = localData[ldID]->GetFaceSel();
			if((!faceSel.IsEmpty()) && mMod->WtIsChecked(ID_LSCM_INTERACTIVE))
			{
				// In peel mode, we set the face peel BitArray
				localData[ldID]->AddFacePeelSel(faceSel);
				mMod->SetFacePeeled(true);
			}
