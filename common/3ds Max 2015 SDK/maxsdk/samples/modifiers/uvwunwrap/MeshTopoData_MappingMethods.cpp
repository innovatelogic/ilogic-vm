...
void MeshTopoData::DetachFromGeoFaces(const BitArray& faceSel, BitArray &vertSel, UnwrapMod *mod)
{
...
void	MeshTopoData::AlignCluster(Tab<ClusterClass*> &clusterList, int moveCluster, int innerFaceIndex, int outerFaceIndex,int edgeIndex,UnwrapMod *mod)
{
...
BOOL	MeshTopoData::BuildCluster(const Tab<Point3>& normalList, float threshold, BOOL connected, BOOL cleanUpStrayFaces, MeshTopoData::GroupBy groupBy, Tab<ClusterClass*> &clusterList, UnwrapMod *mod)
{
	//If the TVMaps has not been set yet
	if( (TVMaps.f.Count() == 0)  || (mFSel.GetSize() != TVMaps.f.Count()) )
	{
		DbgAssert(0);
		return FALSE;
	}
...
void MeshTopoData::ApplyMap(int mapType,  BOOL normalizeMap, const Matrix3& gizmoTM, UnwrapMod *mod)
{
