...
public:
	...
	//this returns the current peeling face collection for this local data
	BitArray& GetFacePeelSel() {return mFacePeel; }
	//this copies a peel face collection into our peel face collection
	void SetFacePeelSel(BitArray& set);
	//this adds more face selected into peel faces collection
	void AddFacePeelSel(BitArray& set);
...
//TV Face Methods*********************************************************************
	...
	//gets/sets whether a face is peeling
	BOOL GetFacePeeledAt(int faceIndex);
	void SetFacePeeledAt(int faceIndex, BOOL sel);
...
//breaks the UV edges
	...
	void DetachFromGeoFaces(const BitArray& faceSel, BitArray &vertSel, UnwrapMod *mod);
...
//several relax algorithms
	...
	BOOL	BuildCluster( const Tab<Point3>& normalList, float threshold, BOOL connected, BOOL cleanUpStrayFaces,  MeshTopoData::GroupBy groupBy, Tab<ClusterClass*> &clusterList, UnwrapMod *mod);
	void	AlignCluster(Tab<ClusterClass*> &clusterList, int moveCluster, int innerFaceIndex, int outerFaceIndex,int edgeIndex,UnwrapMod *mod);
	...
	void ApplyMap(int mapType, BOOL normalizeMap, const Matrix3& tm, UnwrapMod *mod);
...
//does a quick intersect test on a specific face
	...
	//this is the peeling face selection
	BitArray mFacePeel;
