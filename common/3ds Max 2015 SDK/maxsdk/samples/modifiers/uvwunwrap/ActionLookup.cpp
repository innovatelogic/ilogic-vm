...
//TOOLS
	...
	case ID_WELDTHRESH_EDIT:
		{
			...
			itoolBar->AddTool(ToolOtherItem(CUSTEDITWINDOWCLASS, 30,	16, ID_WELDTHRESH_EDIT,WS_CHILD|WS_VISIBLE, 0, NULL, 0));
			itoolBar->AddTool(ToolOtherItem(SPINNERWINDOWCLASS, 18,	16, ID_WELDTHRESH_SPINNER,WS_CHILD|WS_VISIBLE, 0, NULL, 0));
			ISpinnerControl* uSpin = GetISpinner(itoolBar->GetItemHwnd(ID_WELDTHRESH_SPINNER));
			uSpin->LinkToEdit(itoolBar->GetItemHwnd(ID_WELDTHRESH_EDIT),EDITTYPE_FLOAT);
			uSpin->SetLimits(0.0f, 10.0f, FALSE);
			uSpin->SetValue(fnGetWeldThresold(),FALSE);
			uSpin->SetScale(0.1);
			ReleaseISpinner(uSpin);
	...
	case ID_PEEL_DETACH:
		{
			ICustToolbar* itoolBar = toolBar->GetToolBar();
			itoolBar->AddTool(ToolOtherItem(_T("BUTTON"), 62,	mToolBarHeight, ID_PEEL_DETACH,WS_CHILD|WS_VISIBLE|BS_CHECKBOX|BS_VCENTER, 0, GetString(IDS_EMPTY), 0));						
