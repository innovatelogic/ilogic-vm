...
BOOL UnwrapMod::CollapseClusters(float spacing, BOOL rotateClusters)
{
...
BOOL UnwrapMod::LayoutClusters2(float spacing, BOOL rotateClusters, BOOL combineClusters)
{
	...
	if (combineClusters)
	{
		if (!CollapseClusters(spacing,rotateClusters)) return FALSE;
	}
...
//						TVMaps.cont[j]->SetValue(0,&TVMaps.v[j].p);
	...
	statusMessage.printf(_T("Done, area coverage %3.2f"),(fABArea)*100.f);
	   ...
	   for (int i = 0; i < mMeshTopoData.Count(); i++)
	   {
		   ...
		   while (numberFaces != usedFaces.NumberSet())
		   {
			   faces.SetCount(0);
				...
				//get the start index
				BitArray selFaces = ld->GetFaceSelection();
				...
				{
					if (ld->GetFaceSelected(i) && (usedFaces[i] == false))
					{
						faces.Append(1,&i,10000);
					}
				}
				usedFaces |= ld->GetFaceSelection();

				ld->SetFaceSelection(selFaces);
