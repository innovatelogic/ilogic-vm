...
void MeshTopoData::Reset(ObjectState *os, int channel)
{
	...
	mFacePeel.SetSize(TVMaps.f.Count());
	mFacePeel.ClearAll();
...
void MeshTopoData::SetFacePeelSel(BitArray& set)
{
	if (set.GetSize() != TVMaps.f.Count())
	{
		DbgAssert(0);
...
void MeshTopoData::AddFacePeelSel(BitArray& set)
{
	if (set.GetSize() != TVMaps.f.Count())
	{
		DbgAssert(0);
...
BOOL MeshTopoData::GetFacePeeledAt(int faceIndex)
{
	if(mFacePeel.IsEmpty())
	{
		return FALSE;
	}
	if ((faceIndex < 0) || (faceIndex >= TVMaps.f.Count()) || (faceIndex >= mFacePeel.GetSize()))
	{
		DbgAssert(0);
		return FALSE;
	}

	return mFacePeel[faceIndex];
}

void MeshTopoData::SetFacePeeledAt(int faceIndex, BOOL sel)
{
	if ((faceIndex < 0) || (faceIndex >= TVMaps.f.Count()) || (faceIndex >= mFacePeel.GetSize()))
	{
		DbgAssert(0);
...
void MeshTopoData::Stitch(BOOL bAlign, float fBias, BOOL bScale, UnwrapMod *mod)
{
	...
	if (mVSel.NumberSet() == 1)
	{
		...
		{        
			...
			{
				if (IsTVVertVisible(potentialVertex))
				{
	...
	{
		TVConnectionInfo connectionData(this);
		int firstIndex = -1;
		for (int i = 0; i < TVMaps.v.Count(); i++)
		{
			if (mVSel[i])
