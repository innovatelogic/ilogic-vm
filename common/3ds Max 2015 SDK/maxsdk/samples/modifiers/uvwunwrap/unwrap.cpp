...
bool UnwrapMod::GetFacePeeled() const
{
	return mbFacePeeled;
}

void UnwrapMod::SetFacePeeled(bool bFacePeeled)
{
	if(mbFacePeeled != bFacePeeled)
	{
		mbFacePeeled = bFacePeeled;
	}
}

void UnwrapMod::ClearFacePeelBitArray()
{
	// when exiting peel mode, the BitArray of peeled faces need to set as zero
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		if(mMeshTopoData[ldID])
		{
			mMeshTopoData[ldID]->GetFacePeelSel().ClearAll();
		}
	}
	SetFacePeeled(false);
}
