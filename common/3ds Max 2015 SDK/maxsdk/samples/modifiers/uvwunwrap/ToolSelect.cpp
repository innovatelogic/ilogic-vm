...
void UnwrapMod::fnGeomLoopSelect()
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		edgesAtVertex.SetCount(ld->GetNumberGeomVerts());
		for (int i = 0; i < ld->GetNumberGeomVerts(); i++)
			edgesAtVertex[i] = NULL;

		BitArray vertsConnectedToDegenerateFace;
		vertsConnectedToDegenerateFace.SetSize(ld->GetNumberGeomVerts());
		...
		for (int i = 0; i < ld->GetNumberFaces(); i++)
		{
			...
			for (int j = 0; j < deg; j++)
			{
				int vertIndex0 = ld->GetFaceGeomVert(i,j);
				int vertIndex1 = ld->GetFaceGeomVert(i,(j+1)%deg);
			...
			if (degenerateFace)
			{
				for (int j = 0; j < deg; j++)
				{
					int vertIndex = ld->GetFaceGeomVert(i,j);
				...
				for (int j = 0; j < 2; j++)
				{
					...
					while (!done)
					{
						...
						for (int k = 0; k < edgesAtVertex[starVert]->edgeIndex.Count(); k++)
						{
							...
							int b = ld->GetGeomEdgeVert(eindex,1);
							if (a == b)
								degenFound = TRUE;
							if (ld->GetGeomEdgeNumberOfConnectedFaces(eindex) == 1)
								openEdge = TRUE;
							if (!( ld->GetGeomEdgeHidden(eindex)))
