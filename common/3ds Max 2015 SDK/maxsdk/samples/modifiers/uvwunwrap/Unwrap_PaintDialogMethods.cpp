...
void UnwrapMod::PaintFaces(HDC hdc, bool bInPeelMode)
{
	if(bInPeelMode && !GetFacePeeled())
	{
		// In Peel Mode, if there are no faces peeled, we exit paint faces.
		// If there are faces peeled, go on painting them.
		return;
	}
	
	HPEN selPen   = CreatePen(PS_SOLID, 2, bInPeelMode ? peelColor : selColor);
	HPEN selEdgePen = CreatePen(PS_SOLID, 2, selColor);
	HPEN selEdgeThinPen = CreatePen(PS_DOT, 1, selColor);
		...
		for (int whichLD = 0; whichLD < mMeshTopoData.Count(); whichLD++)
		{
			...
			if (ld)
			{
				for (int i=0; i< ld->GetNumberFaces(); i++) 
				{
					...
					if(bInPeelMode)
					{
						bDraw = ld->IsFaceVisible(i) && (ld->GetFacePeeledAt(i)) ;
						if(bDraw && (mTVSubObjectMode == TVFACEMODE) && (ld->GetFaceSelected(i)))
						{
							// The selection faces had already been drawn in select color,
							// so there is no need to draw them in Peel mode
							bDraw = false;
						}
					}
					else
					{
						bDraw = ld->IsFaceVisible(i) && (mTVSubObjectMode == TVFACEMODE) && (ld->GetFaceSelected(i)) ;
					}
				...
				// In Peel Mode, we need to paint the selected edges as "selColor" color on the colored peeled faces
				if(bInPeelMode && (mTVSubObjectMode == TVEDGEMODE))
				{
					for(int i=0; i< ld->GetNumberTVEdges(); i++)
					{
						if(ld->GetTVEdgeSelected(i))
						{
							...
							{
								int veca = ld->GetTVEdgeVec(i, 0);
								int vecb = ld->GetTVEdgeVec(i, 1);
								BOOL hiddenEdge = FALSE;
								int numberFacesAtThisEdge = ld->GetTVEdgeNumberTVFaces(i);
								if((!displayHiddenEdges) && (numberFacesAtThisEdge > 1))
								{
								...
								if(hiddenEdge)
								{
									SelectObject(hdc, selEdgeThinPen);
								}
								else
								{
									SelectObject(hdc, selEdgePen);
								}
								// draw the selected edges
								DrawEdge(hdc, veca, vecb, ld->GetTransformedPoint(a), ld->GetTransformedPoint(b), 
									ld->GetTransformedPoint(veca), ld->GetTransformedPoint(vecb));
							}
