...
public:
	...
	BOOL reset;
	PanMode(UnwrapMod *m) {mod=m;reset=FALSE;}
...
//	BOOL fnGetQMapAlign();
	...
	void PaintFaces(HDC hdc, bool bInPeelMode);
	...
	****************************************************************************************/
	...
	BOOL	CollapseClusters(float spacing, BOOL rotateClusters);
...
//	BOOL	AddActionToToolbar(ICustToolbar *toolBar, int id);
	...
	//if mbPeelDetach is true, uvw editor will firstly execute break operation for Quick Peel / Peel Mode operations. Vice versa.
	bool mbPeelDetach;
	...
	//Get/Set whether the faces were peeled
	bool GetFacePeeled() const;
	void SetFacePeeled(bool bFacePeeled);
	void ClearFacePeelBitArray();
