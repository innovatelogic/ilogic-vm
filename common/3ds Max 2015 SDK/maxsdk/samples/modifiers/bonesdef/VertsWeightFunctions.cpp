...
void BonesDefMod::SetVertex(BoneModData *bmd,int vertID, int boneIndex, float amount)
{
	if (!ip)
		return;
...
void BonesDefMod::SetVertices(BoneModData *bmd,int vertID, const Tab<int>& boneIndexList, const Tab<float>& amountList)
{
	if (!ip)
		return;
...
void BonesDefMod::BuildNormalizedWeight(BoneModData *bmd)
{
	...
	#pragma omp parallel for schedule(static, dataCount / numThreads)
