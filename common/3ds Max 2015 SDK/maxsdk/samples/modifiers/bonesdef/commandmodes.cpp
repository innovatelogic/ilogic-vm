...
static BonesDefMod* get_bonedef_mod(Value* arg)
{
	...
	if ( !bmod->ip )
		throw RuntimeError(GetString(IDS_PW_SKIN_NOT_SELECTED), arg);
	return bmod;
}
...
static int ConvertBoneIDToBoneIndex(BonesDefMod* bmod, int boneID)
{
	...
	if (boneIndex == -1)
		throw RuntimeError(_T("Bone ID out of range: "), Integer::intern(boneID+1));
	return boneIndex;
}
...
Value* selectVerticesByBone_cf(Value** arg_list, int count)
{
	...
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
{
	check_arg_count(bakeSelectedVerts, 1, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
Value* SaveEnvelopeAsASCII_cf(Value** arg_list, int count)
{
	...
	else check_arg_count(SaveEnvelopeAsASCII, 1, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
Value* loadEnvelopeAsASCII_cf(Value** arg_list, int count)
{
	...
	else check_arg_count(loadEnvelopeAsASCII, 1, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
Value* getBoneIDByListID_cf(Value** arg_list, int count)
{
	...
	check_arg_count(getBoneIDByListID, 2, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
Value* getListIDByBoneID_cf(Value** arg_list, int count)
{
	...
	return Integer::intern(listID+1);
}
...
{
	//skinops.getVertexWeight $.modifiers[#Skin] vertexID vertex_bone
...
{
	//skinops.getVertexWeightBoneID $.modifiers[#Skin] vertexID vertex_bone
...
Value* selectVertices_cf(Value** arg_list, int count)
{
	...
	check_arg_count(SelectVertices, 2, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
	...
	if (objects > 0)
	{
		...
		{
			BitArrayValue *list = (BitArrayValue *) ival;
			for (vindex = 0; vindex < list->bits.GetSize(); vindex++)
			{
				if (list->bits[vindex])
				{
					if ( (vindex < 0 ) || (vindex >= bmd->VertexData.Count()) )
...
Value* setVertexWeights_cf(Value** arg_list, int count)
{
	...
	check_arg_count(setVertexWeights, 4, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
	...
	if (objects != 0)
	{
		...
		else // array of indexes
		{
			...
			{
				int boneID = bval->data[i]->to_int()-1;
				int boneIndex = ConvertBoneIDToBoneIndex(bmod, boneID);
...
Value* replaceVertexWeights_cf(Value** arg_list, int count)
{
	...
	check_arg_count(replaceVertexWeights, 4, count);
	BonesDefMod* bmod = get_bonedef_mod(arg_list[0]);
...
Value* setInnerRadius_cf(Value** arg_list, int count)
{
	...
	int boneIndex = ConvertBoneIDToBoneIndex(bmod, boneID);
	int crossID = arg_list[2]->to_int()-1;
	float radius = arg_list[2]->to_float();
...
void BonesDefMod::AddBone(INode *node, BOOL update)
{
	...
	for (int j = 0; j < subcount; j++)
	{
		...
		if (current != -1) {
			...
			boneData.InitObjectTM = otm;
			boneData.tm    = Inverse(otm);
			if (hasStretchTM)
				boneData.InitNodeTM = ntm;
			else boneData.InitNodeTM = stretchTM * ntm;
