...
static bool DrawTextArrow(HDC& hdc, Rect& rect, bool bUp)
{
	COLORREF oldTextColor = SetTextColor(hdc, GetCustSysColor(COLOR_BTNTEXT));
	TSTR str = GetString(IDS_BONEDEF_NAME);
	// caculate the space we need to display text and arrow.
	SIZE sz;
	DLGetTextExtent(hdc,str.data(),&sz);
	int xPos = rect.left + LEFT_MARGIN_BTN_TEXT; // default to left-align.
	int rectHeight = rect.bottom - rect.top;
	int yPos = rect.top + TOP_MARGIN_BTN_TEXT;
	if (sz.cy <= rectHeight)
	{
		yPos = rect.top + (rectHeight >> 1) - (sz.cy >> 1); // try put it center.
	}
	DLTextOut( hdc, xPos, yPos, str.data());
	xPos = rect.right - ARROW_IMAGE_WIDTH - RIGHT_MARGIN_ARROW_BTN;
	yPos = rect.top + TOP_MARGIN_ARROW_BTN;
	if (ARROW_IMAGE_HEIGHT <= rectHeight) // try put it center.
	{
		yPos = rect.top + (rectHeight >> 1) - (ARROW_IMAGE_HEIGHT >> 1);
	}
	ImageList_Draw(hArrowImages,
		bUp?UP_ARROW_INDEX:DOWN_ARROW_INDEX,
		hdc, xPos, yPos, ILD_NORMAL);
