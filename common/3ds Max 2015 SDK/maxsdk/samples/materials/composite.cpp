	...
	if (!m_DisplayTexValidInterval.InInterval(t) || colorCorrectionModeChanged)
	{
		...
		for( ; it != end(); ++it ) 
		{
			if (it->IsVisible()) 
			{
				...
				if (CreateViewportDisplayBitmap(bmi, t, cb, tempInterval, it, bmiDiffuse != NULL, forceWidth, forceHeight) && bmi)
				{
					...
					if (!bmiDiffuse)
					{
						...
						forceWidth = bmiDiffuse->bmiHeader.biWidth;
						forceHeight = bmiDiffuse->bmiHeader.biHeight;
					}
					else
					{	//combine the existing bmiDiffuse with the new layer bitmap if possible.
						Texmap* pTexmap = it->Texture();
						if (IsSameSize(bmi, bmiDiffuse))
						{
	...
	if (m_DisplayTexHandle)
	{
		...
		for( ; it != end(); ++it )
		{
			...
			{	// use the uv setting of the first visible map.
				...
				cb.UpdateTextureMapInfo( t, ISimpleMaterial::UsageDiffuse, pSubMap);
				break;
			}
		}
