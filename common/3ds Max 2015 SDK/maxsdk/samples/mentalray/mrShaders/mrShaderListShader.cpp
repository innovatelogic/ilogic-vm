...
void mrShaderListShader_PostOpenCallback(void* receiver, NotifyInfo* info)
{
	mrShaderListShader *pShaderListShader = (mrShaderListShader *)receiver;
	pShaderListShader->PostOpen();
}
...
{
	RegisterNotification(mrShaderListShader_PostOpenCallback, this, NOTIFY_FILE_POST_OPEN);
...
mrShaderListShader::~mrShaderListShader()
{
	UnRegisterNotification(mrShaderListShader_PostOpenCallback, this);
...
void mrShaderListShader::AppendShader(Texmap* shader, bool on) {
	...
	// [Elvic 5/25/2012] Don't append the shader if it's NULL
	if (shader == NULL) {
		return;
	}
...
RefResult mrShaderListShader::NotifyRefChanged( const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate ) {
...
// [Elvic 6/26/2012] Now PostLoadCallback::proc is not guaranteed to be called 
// after the entire scene loading is done, instead, some parameters may be loaded 
// and updated after PostLoadCallback::proc. So, we should register a post scene 
// open notification to do something after all scene elements are loaded.
void mrShaderListShader::PostOpen()
{
	// Accept changes in the active shader
	m_dontCommitActiveShader = false;

	// Set the selected item according to the current shader in Texmap after loading, 
	// thus we make sure the current shader is selected in the list box.
	if (m_mainPB != NULL)
	{
		Texmap* activeShader = m_mainPB->GetTexmap(kMainPID_CurrentShader);
		for (int i = 0; i < GetNumShaders(); ++i)
		{
			if (GetShader(i) == activeShader)
			{
				SetSelectedItem(i);
				break;
			}
