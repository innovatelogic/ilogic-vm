...
class CylinderParamDlgProc : public ParamMapUserDlgProc
{
private:
	CylinderObject* mCylinder;
	AnimHandle mCylinderHandle;
	HWND thishWnd;
public:
	CylinderParamDlgProc(CylinderObject* s);
	~CylinderParamDlgProc();
	INT_PTR DlgProc(TimeValue t,IParamMap* map,HWND hWnd, UINT msg, WPARAM wParam,LPARAM lParam);
...
{
	mCylinder = s;
	mCylinderHandle = Animatable::GetHandleByAnim(s);
	thishWnd = NULL;
}
CylinderParamDlgProc::~CylinderParamDlgProc()
{
	mCylinder = NULL;
	thishWnd = NULL;
}
...
void CylinderParamDlgProc::Update(TimeValue t)
{
	if (!thishWnd) 
		return;
	int ison;
	DbgAssert( Animatable::GetAnimByHandle(mCylinderHandle) != NULL );
	mCylinder->pblock->GetValue(PB_SLICEON,t,ison,FOREVER);
...
void CylinderParamDlgProc::TurnSpinner( HWND hWnd,int SpinNum,BOOL ison )
{
...
void CylinderObject::BeginEditParams(IObjParam *ip,ULONG flags,Animatable *prev)
{
	...
	if (pmapCreate && pmapParam) {
		...
		CylinderParamDlgProc* dlg = static_cast<CylinderParamDlgProc*>(pmapParam->GetUserDlgProc());
		if (dlg != NULL) {
	...
	if(pmapParam) {
		// A callback for the type in.
		pmapParam->SetUserDlgProc(new CylinderParamDlgProc(this));
	}
