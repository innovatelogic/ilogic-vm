...
static MaxSDK::Graphics::Utilities::MeshEdgeKey TargetKey;
class TargetMeshItem : public MaxSDK::Graphics::Utilities::MeshEdgeRenderItem
{
public:
	TargetMeshItem(Mesh* pMesh)
		: MeshEdgeRenderItem(pMesh, true, false)
	{

	}
	virtual ~TargetMeshItem()
	{
	}
	virtual void Display(MaxSDK::Graphics::DrawContext& drawContext)
	{
		ViewExp* vpt = const_cast<ViewExp*>(drawContext.GetViewExp());
		if (nullptr != vpt && nullptr != vpt->GetViewCamera()
			&& vpt->GetViewCamera()->GetTarget() == drawContext.GetCurrentNode())
		{
			return;
		}
								 ...
								 MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	MaxSDK::Graphics::Utilities::MeshEdgeRenderItem* pMeshItem = new TargetMeshItem(&mesh);
	if (pNode->Dependent())
	{
	...
	else if(pNode->GetLookatNode())
	{
		const ObjectState& os = pNode->GetLookatNode()->EvalWorldState(updateDisplayContext.GetDisplayTime());
		Object* ob = os.obj;
		if ( (ob!=NULL) && ( (ob->SuperClassID()==LIGHT_CLASS_ID) ||
			(ob->SuperClassID()==CAMERA_CLASS_ID) ) )
		{												
			pMeshItem->SetColor(Color(pNode->GetWireColor()));
		}
		else
		{
			pMeshItem->SetColor(Color(GetUIColor(COLOR_CAMERA_OBJ)));
		}
	}
	...
	if (nullptr != vpt.GetViewCamera()
		&& vpt.GetViewCamera()->GetTarget() != pNode)
	{
...
bool TargetObject::UpdatePerNodeItems(
	...
	MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	AddRenderitem(updateDisplayContext, nodeContext, targetRenderItemContainer);
	return true;
}
