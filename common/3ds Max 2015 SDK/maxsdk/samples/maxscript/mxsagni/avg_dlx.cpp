...
{
	//	doesFileExist <filename_string> allowDirectory:<bool>
	check_arg_count_with_keys(doesFileExist, 1, count);
	Value* tmp;
	bool allowDirectory = bool_key_arg(allowDirectory, tmp, TRUE) != FALSE; 
	return (DoesFileExist(arg_list[0]->to_filename(), allowDirectory) ? &true_value : &false_value);
}

Value* 
doesDirectoryExist_cf(Value** arg_list, int count)
{
	//	doesDirectoryExist <dirname_string>
	check_arg_count(doesDirectoryExist, 1, count);
	return (DoesDirectoryExist(arg_list[0]->to_filename()) ? &true_value : &false_value);
}
...
getAnimPointer_cf(Value** arg_list, int count)
{
	...
	if( refTarg == nullptr ) return Integer::intern( 0 );

	return IntegerPtr::intern( (INT_PTR)refTarg );
}
...
get_max_file_asset_metadata_cf(Value** arg_list, int count)
{
	...
	eight_typed_value_locals(Name* struct_name, StructDef* sd, Array* result, Value* struct_instance, Value* assetId, Value* filename, Value* assetType, Value* resolvedFilename);
...
set_max_file_asset_metadata_cf(Value** arg_list, int count)
{
	...
	const TCHAR* scene_dir = MAXScript_interface->GetDir(APP_SCENE_DIR);
	TCHAR  buf[MAX_PATH] = {0};
	six_typed_value_locals(Name* struct_name, StructDef* sd, Value* assetId, Value* filename, Value* assetType, Value* resolvedFilename);
...
Value* timeGetTime_cf(Value** arg_list, int count)
{
	...
	return_protected(Integer64::intern(timeGetTime()));
}
...
def_visible_primitive	( SRM_shutdown_test_function,		"SRM_shutdown_test");
*/
...
def_visible_primitive	( ValidateReferenceMakerRefs,		"ValidateReferenceMakerRefs");
