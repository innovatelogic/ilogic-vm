...
#else // use_original_vertex_paint
BOOL ApplyVCUtil::ApplyNode(INode* node, MeshInstanceTab& instancelist, bool noUpdate)
#endif   // use_original_vertex_paint
{
...
#else // use_original_vertex_paint
        if (currentOptions.mixVertColors) {
            if (!calcMixedVertexColors(node, ip->GetTime(), currentOptions, mixedVertexColors, instancelist, &fn)) {
                ip->ProgressEnd();
                return FALSE;
            }
        }
        else {
            if (!calcFaceColors(node, ip->GetTime(), currentOptions, faceColors, instancelist, &fn)) {
                ip->ProgressEnd();
                return FALSE;
            }
        }
