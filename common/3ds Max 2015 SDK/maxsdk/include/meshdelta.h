...
public:
	...
	*/
	...
	\param UVVert* tv - The map vertex array to change. This should be allocated to handle all the
	new map vertices in the vCreate array.
	\param TVFace* tf - The map face array to change. This should be allocated to handle all the new
	map faces in the fCreate array.
	\param DWORD inVNum - The actual number of map vertices (which doesn't have to match this
	MapDelta's vnum) in the input map.
	\param DWORD inFNum - The actual number of map faces (which doesn't have to match this MapDelta's fnum) 
	in the input map. */
