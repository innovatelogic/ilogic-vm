...
//! \brief Sent after a file open fails.  NotifyInfo::callParam is a value of type FileIOType.
...
/*!  
 *   \callParam MCHAR* - full path to the file being saved
 */
...
/*!  
*   \callParam int - nonzero if the safe frame is displayed in the viewport; otherwise 0
 *   \Note This sets a dirty flag on the view and some values on the ViewExp class
 ...
 *   \callParam MCHAR* - full path to the file that was loaded
 */
#define NOTIFY_CUI_MENUS_POST_LOAD				0x000000F5

//! \brief Sent after the parent of a layer was changed.
/*! callParam is a pointer on the struct LayerParentChange {ILayer *layerChanged;ILayer *oldParent;};. 
...
callParam is a pointer the ActionItem instance
 */
...
callParam is a pointer to the ClassDesc instance describing the plug-in whose instance is created.
*/
...
/*! This notification is not sent when plug-in instances are created programmatically (SDK, Maxscript, python etc).
callParam is a pointer to the ClassDesc instance describing the plug-in whose instance has been created.
