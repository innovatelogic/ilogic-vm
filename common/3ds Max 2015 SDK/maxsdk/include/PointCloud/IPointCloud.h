...
/** The ID can be used to return the interface pointer through the <b>GetInterface()</b> method of classes 
...
#define IID_POINT_CLOUD Interface_ID(0x181a2a1c, 0x34ea4e72)
	...
	enum PointCloudPBIds
	{
		...
		pb_solidColor, //!< color used when display technique is solid color mode
		...
		pb_fence_objects, //!< TYPE_INODE_TAB the nodes which are used as display volumes (only support box, sphere and plane)
	...
	/*! This interface provides various methods to interact with point cloud objects. For example, it provides a method to 
		perform	hit tests on point cloud objects and it also provides a method to get information of all visible point cloud
		vertices from a specific view. 
		Users could get this interface by calling GetInterface(IID_POINT_CLOUD) method of classes which implement 
		this interface.
		\code
	...
	*/
	...
	{
	public:		
		//! \brief Creates IPointCloudVisibleVoxelNode array based on the current point cloud settings and input parameters.
		//! Then users could use the methods of IPointCloudVisibleVoxelNode to get points' information(position, normal, color, etc.)
		//! After using them, user should be responsible to destroy the IPointCloudVisibleVoxelNode in the array.
		//! \param[in] t - time 
		...
		//! \brief Return the visible point count in a specific viewport
		virtual unsigned long GetVisiblePointCount(ViewExp& vpt) = 0;

		/*!	 Return hit test result for point cloud. If users just want to get whether the hit test ray hits
			the point cloud object or not and no need to get the hit points, the last two parameters of this function
