...
//	#pragma comment( lib, "../common/SDK/nv_math/Lib/Debug/nv_math.lib" )
#else
//	#pragma comment( lib, "../common/SDK/nv_math/Lib/Release/nv_math.lib" )
#endif
                           ...
                           const Option _WeightNormalsByFaceSize )
{
    ...
    if ( bNeedNormals || bComputeTangentSpace )
    {
        ...
        if ( !have_normals )
        {
            ...
            if ( want == outmap.end() )
			{
                ...
                outmap[ "normal" ] = (unsigned int)output.size() - 1;
    ...
    if ( bNeedTexCoords || bComputeTangentSpace )
    {
        if ( outmap.find("tex0") == outmap.end() )
		{
            ...
            outmap[ "tex0" ] = (unsigned int)output.size() - 1;
        }
		...
		if ( _FixCylindricalTexGen == FixCylindricalTexGen )
		{
			...
			const unsigned int theSize = (unsigned int)indices.size();
			...
			for ( unsigned int f = 0; f < theSize; f += 3 )
			{
				for ( int v = 0; v < 3; ++v )
				{
					...
					if ( bDoS == true )
					{
						unsigned int theNewIndex = (unsigned int)texcoords.size() / 3;
	...
	if ( bComputeTangentSpace )
	{
        ...
        if ( want == outmap.end() )
		{
            ...
            outmap[ "tangent" ] = (unsigned int)output.size() - 1;
            want = outmap.find( "tangent" );
        }
		// just initialize array so it's the correct size
		output[ (*want).second ].floatVector_ = positions;
        ...
        if ( want == outmap.end() )
		{
            ...
            outmap[ "binormal" ] = (unsigned int)output.size() - 1;
            want = outmap.find( "binormal" );
        }
		// just initialize array so it's the correct size
	    output[ (*want).second ].floatVector_ = positions;
		...
		for ( unsigned int f = 0; f < theSize; f += 3 )
		{
			...
			float __v0 = tex[indices[f + 1] ].y;
			float __v1 = tex[indices[f]].y;
				...
				for ( int e = 0; e < 3; ++e )
				{
					...
					else
					{
							 ...
							 ( fabs( sxtAgreement ) < epsilon ) )
						{
							...
							const unsigned int theNewIndex = (unsigned int)positions.size() / 3;
        ...
        const int theFaceCount = (unsigned int)indices.size() / 3;
